...
pub struct ProducerConfig {
    ...
    pub stream_queue_length: Option<i8>,
...
impl ProducerPolicy {
    fn from_config(config: &ProducerConfig) -> Result<Self, String> {
        if config.fps == 0 {
            return Err("fps must be greater than zero".to_owned());
        }

        let profile = ProfileDefaults::new(config.profile, config.fps);
        let quality = config.quality.unwrap_or(profile.quality);
        ...
        let keyframe_interval = config
            ...
            .max(1);
        let capture_queue_depth = config
            .capture_queue_depth
            .unwrap_or(profile.capture_queue_depth)
            .clamp(1, 8);
        let stream_queue_length = config
            .stream_queue_length
        ...
        Ok(Self {
            ...
            preserve_aspect_ratio: !config.stretch,
        })
    }
}
...
pub async fn run_producer(config: ProducerConfig) -> Result<(), Box<dyn std::error::Error>> {
    let policy = ProducerPolicy::from_config(&config).map_err(invalid_input)?;

    let display = capture::Display::primary();
    let (cap_width, cap_height) = target_resolution(&config, &display).map_err(invalid_input)?;
    ...
    let ws_url = match config.token.as_deref().filter(|token| !token.is_empty()) {
        Some(token) => format!("{ingest_url}?token={token}"),
        None => ingest_url,
    };
...
fn target_resolution(
    ...
    let (preset_width, preset_height) = match config.resolution {
